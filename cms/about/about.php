...
$about_edu->years = sanitizeInput($_POST['years'], 'string') ?: ($existing_edu->years ?? null);
    ...
    if (empty($errors)) {
        try {
            ...
            // Update or insert experience information
            $stmt = $conn->prepare("INSERT INTO about_exp (exp_years, exp_field, about_me, profile_id) 
                                    VALUES (:exp_years, :exp_field, :about_me, :profile_id) 
                                    ON DUPLICATE KEY UPDATE 
                                    exp_years = VALUES(exp_years), 
                                    exp_field = VALUES(exp_field), 
            ...
            // Update or insert education information
            $stm = $conn->prepare("INSERT INTO about_edu (level, certificate, years, profile_id) 
                                   VALUES (:level, :certificate, :years, :profile_id) 
                                   ON DUPLICATE KEY UPDATE 
                                   level = VALUES(level), 
...
<h1>Manage About Information</h1>
